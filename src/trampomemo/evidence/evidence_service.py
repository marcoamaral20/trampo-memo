...
from trampomemo.evidence.memory_search import ALGORITHM_NAME, DeterministicMemorySearch
...
class EvidenceService:
    def __init__(
        ...
        *,
        ...
        memory_search: DeterministicMemorySearch | None = None,
    ) -> None:
        self.memory_repository = memory_repository
        self.chunk_repository = chunk_repository
        self.evidence_repository = evidence_repository
        self.provider = provider or DeterministicLocalEmbeddingProvider()
        self.memory_search = memory_search or DeterministicMemorySearch()
    ...
    def construct_evidence(self, *, question: str, limit: int) -> list[Evidence]:
        ...
        memories = self.memory_repository.list_all()
        chunks_by_id = {
            memory.chunk_id: chunk
            for memory in memories
            if (chunk := self.chunk_repository.get(memory.chunk_id)) is not None
        }
        candidates = self.memory_search.search(
            question=normalized_question,
            question_vector=provider_result.vector,
            memories=memories,
